...
class PersonalButlerAgent:
    ...
    # Syncronous initialization of components
    def __init__(
        self,
    ...
    # Asyncronous initialization of components
    async def _async_init_components(
        self, remote_agent_addresses: list[str]
    ) -> None:
        """Asynchronous part of initialization."""
        # This class will use asyncronous methods in order to obtain the json data related to agent adresses provided. 
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            for address in remote_agent_addresses:
                ...
                try:
                    ...
                    #Populate internal dictionary with RemoteAgentConnection objects
                    self.remote_agent_connections[card.name] = remote_connection
                    #Populate internal dictionary with AgentCard objects
    ...
    # Asyncronous factory for self, wrapper for __init__() and _async_init_components.
    @classmethod
    async def create(
        cls,
        ...
        # (This is just fancy hack to add sync capabilities afaik.)
        """Create and asynchronously initialize an instance of the RoutingAgent."""
        instance = cls(task_callback)
        await instance._async_init_components(remote_agent_addresses)
        return instance

    #Creates an ADK agent not a instance of self. Yes this is confusing.
    #Factory which Converts a RoutingAgent into an ADK Agent for usage.
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""
        **Role:** You are a personal butler. Your primary function is to accurately respond to user requests and solve user problems by delegating inquiries to the appropiate specialized remote agents.
    ...
    # determine whether or not ther is a currently active remote agent by inspectin the conversation state. 
    ...
    # Returns a list of dictionaries which contain the card names and descriptions for a handful of agents. THis will later be appended together as a string to feed the agent description.
...
# This is a wrapper which returns a fully initialized ADK Agent class initialized and ready to go
def _get_initialized_routing_agent_sync() -> Agent:
    ...
    async def _async_main() -> Agent:
        agent_instance = await PersonalButlerAgent.create(
    ...
    try:
        #have to hook onto loop because adk will start one on its own somewhere.
        return asyncio.run(_async_main())
        loop = asyncio.get_running_loop()
        future = asyncio.ensure_future(_async_main())
        return asyncio.get_event_loop().run_until_complete(future)
    except RuntimeError as e:
        # No event loop is running, safe to run normally
        print(e)
        return
        return asyncio.run(_async_main())
        ...
        if 'asyncio.run() cannot be called from a running event loop' in str(e):
            print(
                f'Warning: Could not initialize ButlerAgent with asyncio.run(): {e}. '
                'This can happen if an event loop is already running (e.g., in Jupyter). '
                'Consider initializing RoutingAgent within an async function in your application.'
            )
        raise
personal_butler_root = _get_initialized_routing_agent_sync()
if not personal_butler_root:
    raise RuntimeError("Failed to initialize Personal Butler Agent.")

root_agent = personal_butler_root
